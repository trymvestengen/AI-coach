// ExerciseDetail.jsx — deep screen for a single exercise
// Header illustration with muscle highlight, tabs (Summary/History/How to),
// PR graph, description, coach tips.

function MuscleBody({ highlight = [], width = 140, view = 'front' }) {
  // Simple anatomical silhouette — front + back. Placeholder-quality but clean.
  // highlight: array of muscle keys to tint. Keys: chest, shoulders, triceps, biceps,
  // upperBack, lats, lowerBack, abs, glutes, quads, hamstrings, calves, forearms
  const on = (k) => highlight.includes(k);
  const accent = 'var(--accent)';
  const body = 'rgba(255,255,255,0.12)';
  const out = 'rgba(255,255,255,0.25)';

  return (
    <svg width={width} height={width * 1.5} viewBox="0 0 100 150" fill="none">
      {/* head */}
      <circle cx="50" cy="12" r="7.5" fill={body} stroke={out} strokeWidth="0.7"/>
      {/* torso */}
      <path d="M36 22 Q50 24 64 22 L68 36 Q70 48 68 62 Q65 74 62 82 L38 82 Q35 74 32 62 Q30 48 32 36 Z"
        fill={body} stroke={out} strokeWidth="0.7"/>
      {/* arms upper */}
      ...
      {view === 'front' && (
        <>
          {on('chest') && (
            <path d="M38 30 Q50 34 62 30 L62 44 Q50 47 38 44 Z" fill={accent} opacity="0.72"/>
          )}
          {on('shoulders') && (
            <>
          ...
          {on('abs') && (
            <path d="M42 48 L58 48 L56 78 L44 78 Z" fill={accent} opacity="0.58"/>
          )}
          {on('biceps') && (
            <>
      ...
      {view === 'back' && (
        <>
          {on('upperBack') && (
            <path d="M36 26 Q50 30 64 26 L66 42 Q50 44 34 42 Z" fill={accent} opacity="0.72"/>
          )}
          {on('lats') && (
            <path d="M34 42 Q50 48 66 42 L64 60 Q50 63 36 60 Z" fill={accent} opacity="0.7"/>
          )}
          {on('lowerBack') && (
            <path d="M40 62 L60 62 L58 78 L42 78 Z" fill={accent} opacity="0.6"/>
          )}
          {on('glutes') && (
            <path d="M38 82 L62 82 L62 94 Q50 97 38 94 Z" fill={accent} opacity="0.72"/>
          )}
          {on('hamstrings') && (
            <>
...
// Simple exercise illustration placeholder — shows a lifter silhouette doing the movement
function ExerciseIllustration({ exerciseKey = 'bench', highlight = ['chest','shoulders','triceps'] }) {
  return (
    <div style={{
      position: 'relative',
      ...
      {/* Front + back muscle diagrams */}
      <MuscleBody width={100} view="front" highlight={highlight}/>
      <MuscleBody width={100} view="back" highlight={highlight}/>
      {/* Play badge — suggests animated illustration */}
      <button style={{
        position: 'absolute', top: 14, right: 14,
        width: 34, height: 34, borderRadius: 999,
        background: 'rgba(10,10,11,0.7)', border: '1px solid var(--border-2)',
...
function ExerciseDetail({ onBack, onLog, exercise }) {
  ...
  const ex = exercise || {
    ...
    pr: '95 kg × 3',
    lastUsed: '2 days ago',
    description: 'Compound pushing movement. Drive the bar off your chest with the full pec and front delt, locking out with triceps. Keep shoulder blades pinched, feet planted.',
    tips: [
      'Bar path should curve slightly back toward your face on the way up.',
      'Touch ~1–2 cm above the nipple line, not the sternum.',
      'Set up with a small arch — ribs down, butt on the bench.',
    ],
  };
  ...
  return (
    <div className="screen">
      ...
      <div style={{
        ...
        <div className="title-m">{ex.name}</div>
        <button style={{
          width: 40, height: 40, borderRadius: 999,
          background: 'transparent', border: 'none', color: 'var(--fg-0)',
          display: 'grid', placeItems: 'center', cursor: 'pointer',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 160px' }}>
        ...
        {tab === 'summary' && (
          <>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.6, textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 2 }}>
                  Coach tip
                </div>
                <div style={{ fontSize: 13, lineHeight: 1.4, color: 'var(--fg-0)', textWrap: 'pretty' }}>
                  Based on your last three sessions, try {ex.pr ? '82.5 kg × 5' : 'a working weight 2.5 kg above last'} today. Your bar speed's been strong.
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
              <div className="card" style={{ padding: 12 }}>
                ...
                <div className="metric" style={{ marginTop: 8, fontSize: 22 }}>{ex.pr}</div>
                <div style={{ fontSize: 11, color: 'var(--fg-3)', marginTop: 4 }}>12 Apr 2026</div>
              </div>
              <div className="card" style={{ padding: 12 }}>
                <div className="caption">Last used</div>
                <div className="metric" style={{ marginTop: 8, fontSize: 22 }}>{ex.lastUsed}</div>
                <div style={{ fontSize: 11, color: 'var(--fg-3)', marginTop: 4 }}>80 kg × 5</div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 14, marginTop: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div>
                  <div className="caption">Heaviest weight</div>
              ...
              {/* Chart */}
              <svg width="100%" height="90" viewBox="0 0 320 90" preserveAspectRatio="none" style={{ display: 'block' }}>
                <defs>
                ...
                {/* gridlines */}
                {[0,1,2,3].map(i => (
                  <line key={i} x1="0" x2="320" y1={i * 22 + 6} y2={i * 22 + 6}
                    stroke="rgba(255,255,255,0.05)" strokeWidth="1"/>
                ))}
                {(() => {
                  const pts = [60,62,58,65,64,68,70,72,74,76,78,80,82.5];
                  const min = 55, max = 85;
                  ...
                  return (
                    <>
                      ...
                      {xs.map((x,i) => i === xs.length-1 && (
                        <circle key={i} cx={x} cy={ys[i]} r="3.5" fill="var(--accent)" stroke="var(--bg-0)" strokeWidth="2"/>
                      ))}
                    </>
                  );
                })()}
            ...
            <div style={{ marginTop: 18 }}>
              <div className="caption" style={{ marginBottom: 6 }}>About</div>
              <div style={{ fontSize: 14, lineHeight: 1.5, color: 'var(--fg-1)', textWrap: 'pretty' }}>
                {ex.description}
              </div>
            </div>
          </>
        ...
        {tab === 'history' && (
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { d: 'Apr 23, 2026', s: '4 sets', best: '82.5 × 5', note: 'PR' },
              { d: 'Apr 16, 2026', s: '4 sets', best: '80 × 5', note: '' },
              { d: 'Apr 9, 2026',  s: '3 sets', best: '80 × 4', note: '' },
              { d: 'Apr 2, 2026',  s: '4 sets', best: '77.5 × 5', note: '' },
              { d: 'Mar 26, 2026', s: '4 sets', best: '75 × 5', note: '' },
            ].map((h, i) => (
              <div key={i} className="card" style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--fg-3)', marginTop: 2 }}>{h.s} · top set {h.best}</div>
                </div>
                {h.note && (
                  <div style={{
        ...
        {tab === 'howto' && (
          <div style={{ marginTop: 14 }}>
            <div className="caption" style={{ marginBottom: 6 }}>Key points</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {ex.tips.map((t, i) => (
                <div key={i} style={{
                  display: 'flex', gap: 12,
                  padding: 12, background: 'var(--bg-2)', border: '1px solid var(--border-1)',
                  borderRadius: 12,
                }}>
                  ...
                  <div style={{ fontSize: 13, lineHeight: 1.45, color: 'var(--fg-1)', textWrap: 'pretty' }}>{t}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{
        ...
        <button onClick={onLog} className="btn-primary" style={{ width: '100%' }}>
          <Icon.Plus size={18}/>
          Log exercise
        </button>
      </div>
...
Object.assign(window, { ExerciseDetail, MuscleBody, ExerciseIllustration });
