...
- [ ] **C-0 Foundation:** scoped forge-komponentlag + token-aliases i `globals.css`. Verifiser: build grønt, eksisterende skjermer uendret (de bruker ikke `.forge`).
- [ ] **C-1 Login** (`login.html`) — enklest, høy synlighet. Valider pipelinen ende-til-ende + screenshot.
- [ ] **C-2 Hjem** (`home.html`) — app-topbar+datebar, greet-row, hero, stat-grid, panel-list m/ plate-icon. (Sosialt forblir skjult.)
- [ ] **C-3 Workout-run** (`workout-run.html`) — forge-tokens i stedet for hardkodet `#1a1a1a`.
- [ ] **C-4 Historikk** (`historikk.html`) + detalj.
