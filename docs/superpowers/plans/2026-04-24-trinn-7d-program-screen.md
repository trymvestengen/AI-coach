# Trinn 7D: Program Screen Redesign — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Erstatt dagens Program-skjerm med det nye Claude Design-layoutet — 12-ukers progress bar, ukestrip (7 dagsfliser), og øktliste med Swap-knapp og progress ring per øvelse.

**Architecture:** To tasks: (1) skriv `ProgramScreen.tsx` med alle sub-komponenter og mock-data, (2) skriv om `program/page.tsx` til å importere ny komponent. Ingen backend-endringer. Eksisterende filer (ProgramList, ProgramDetail, etc.) berøres ikke — kun page.tsx byttes ut.

**Tech Stack:** Next.js 15, TypeScript, CSS custom properties, inline styles + eksisterende CSS utility klasser (.card, .caption, .metric-s, .title-l, .display-l, .tnum, .screen).
...
**Created:**
- `web/src/components/program/ProgramScreen.tsx` — full Program-skjerm med ProgressBar, WeekStrip, ExerciseProgressRing, ExerciseRow

**Modified:**
- `web/src/app/(tabs)/program/page.tsx` — skriv om til å importere ProgramScreen

---

### Task 1: ProgramScreen-komponenten

**Files:**
- Create: `web/src/components/program/ProgramScreen.tsx`

- [ ] **Step 1: Opprett ProgramScreen.tsx**

Opprett `/Users/trymvestengen/Desktop/ai-coach/web/src/components/program/ProgramScreen.tsx` med dette innholdet:
...
interface WeekDay {
  d: string       // "Man", "Tir" etc.
  date: number    // day of month
  exCount: number // 0 = rest
  done: boolean
  today: boolean
}
        ...
        <div style={{ padding: "0 20px" }}>
          <div className="card" style={{ overflow: "hidden" }}>
            ...
            <button style={{
              ...
              background: "none", border: "none", borderTop: "1px solid var(--border-1)",
              cursor: "pointer",
            }}>
...
Forventet: ingen feil. Merk: det er en inline `border` override på "Legg til øvelse"-knappen — TypeScript klager ikke på det, men se note i Step 3.

- [ ] **Step 3: Fiks duplicate border property**

I `ProgramScreen.tsx`, finn knappen "Legg til øvelse" og fjern det duplikate `border: "none"` — bare ha `borderTop`:
...
til:
```tsx
              background: "none", borderTop: "1px solid var(--border-1)",
```

Kjør TypeScript på nytt:
```bash
cd /Users/trymvestengen/Desktop/ai-coach/web && npx tsc --noEmit
```
...
git -C /Users/trymvestengen/Desktop/ai-coach commit -m "feat: add ProgramScreen with week strip, progress bar, exercise list"
```

---

### Task 2: Oppdater program/page.tsx
...
Skriv over `/Users/trymvestengen/Desktop/ai-coach/web/src/app/(tabs)/program/page.tsx` med:
...
```bash
git -C /Users/trymvestengen/Desktop/ai-coach add "web/src/app/(tabs)/program/page.tsx"
git -C /Users/trymvestengen/Desktop/ai-coach commit -m "feat: wire /program page to ProgramScreen"
```
